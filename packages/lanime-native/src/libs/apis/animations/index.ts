    ...
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 0 }) => {
            ...
            return {
                items,
                nextPage:
                    items.length >= PAGE_SIZE ? pageParam + 1 : undefined,
            }
        },
        initialPageParam: 0,
        getNextPageParam: (lastPage) => lastPage.nextPage,
...
export const useAnimationRankings = (type: RankingType) => {
    return useQuery({
        ...
        queryFn: async () => {
            const { data } = await instance.get<
                IApiResponse<RankingAnimation[]>
            >('animations/rankings', { params: { type } })
            return data.data
        },
        staleTime: 1000 * 60 * 5,
    })
    ...
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 0 }) => {
            ...
            >(`animations/${animationId}/similar`, {
                params: { matchPercentage, page: pageParam, limit: PAGE_SIZE },
            })
            return {
                data: data.data ?? [],
